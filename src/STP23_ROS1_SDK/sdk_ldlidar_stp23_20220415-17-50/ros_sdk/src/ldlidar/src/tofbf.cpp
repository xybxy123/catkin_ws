 ...
 */
Tofbf::Tofbf(int speed, LDVersion version) { 
  ...
  switch (version)
  {
    case LDVersion::SIX:
      ...
      break;
    case LDVersion::SIX_P:
      intensity_low_ = 6;
      intensity_single_ = 30;
      scan_frequency_ = 5000;
      break;
    default:
      std::cout << "[ldrobot] tofbf input ldlidar version error!" << std::endl;
      intensity_low_ = 0;
      intensity_single_ = 0;
      scan_frequency_ = 0;
      break;
...
std::vector<PointData> Tofbf::NearFilter(
  ...
  for (auto n : group) {
    ...
    if (n.size() < 3) {
      ...
      c /= n.size();
      if (c < intensity_single_) continue;
    }
