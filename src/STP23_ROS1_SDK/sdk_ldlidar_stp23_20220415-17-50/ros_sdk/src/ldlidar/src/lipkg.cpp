...
LiPkg::LiPkg(std::string frame_id, LDVersion version)
    : timestamp_(0),
      temperature_(0),
      angle_increment_(0),
      error_times_(0),
  ...
  switch (version_)
  {
    case LDVersion::STP23:
      point_frequence_ = 4545;
      break;
    default:
      // std::cout << "[ldrobot] input ldlidar version param error!" << std::endl;
      point_frequence_ = -1; // error
      break;
  }
  // std::cout << "[ldrobot] LDLiDAR measure frequency(Hz) is:" << point_frequence_ << std::endl;
}
...
bool LiPkg::Parse(const uint8_t *data, long len) {
  for (int i = 0; i < len; i++) {
    if (AnalysisOne(data[i])) { // parse a package is success
      temperature_ = pkg.temperature; 
